...
public static class UrlHelper
{
    ...
    )
    {
        string pattern = "\\{(.+?)\\}";
        int pos = 0;
        string formattedPath = Regex.Replace(Path.Combine(baseUrl, apiPath).Replace("\\", "/"), pattern, (Match _) => $"{{{pos++}}}");
    ...
    {
        return string.Join("&", queryParameters.Select<KeyValuePair<string, string>, string>((KeyValuePair<string, string> kvp) =>
