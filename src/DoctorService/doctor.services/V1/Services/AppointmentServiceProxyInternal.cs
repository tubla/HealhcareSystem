...
{
    public async Task<IEnumerable<AppointmentDto>> GetAppointmentsAsync(int doctorId, CancellationToken cancellationToken = default)
    {
        try
        {
            var baseUrl = "http://appointment-service";
            var apiPath = $"api/v1/appointments/doctor/{doctorId}";
